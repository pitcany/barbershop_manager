...
class SchedulingEngine:
    ...
    def _get_day_hours(self, date: datetime) -> Optional[Tuple[time, time]]:
    ...
    def _is_within_business_hours(self, dt: datetime, duration_minutes: int = 30) -> bool:
        """Check if a datetime falls within business hours"""
        # Convert to shop timezone
        local_dt = dt.astimezone(self.shop_tz)
        hours = self._get_day_hours(local_dt)
    ...
    async def validate_appointment_slot(
        ...
        if not self._is_within_business_hours(scheduled_at, duration_minutes):
    ...
    async def get_available_slots(
        ...
        """
        # Convert to shop timezone for business hours check
        local_date = date.astimezone(self.shop_tz)
        hours = self._get_day_hours(local_date)
        
        if not hours:
            return []  # Shop closed this day
        ...
        barbers = await self.db.barbers.find(
            barber_query, {"_id": 0, "id": 1, "name": 1}
        ).to_list(50)
        ...
        # Create datetime for the start of business day
        ...
        # Current time (don't show past slots for today)
