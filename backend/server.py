...
async def seed_demo_data():
    ...
    barbers = [
        {"id": "barber_1", "shop_id": shop_id, "name": "Marcus Johnson", "email": "marcus@classiccuts.local", "active": True, "created_at": now_iso},
        {"id": "barber_2", "shop_id": shop_id, "name": "David Lee", "email": "david@classiccuts.local", "active": True, "created_at": now_iso},
        {"id": "barber_3", "shop_id": shop_id, "name": "Anthony Davis", "email": "anthony@classiccuts.local", "active": True, "created_at": now_iso},
    ]
    await db.barbers.insert_many(barbers)

    # ── Services ────────────────────────────────────────────
    service_defs = [
        {"id": "service_1", "name": "Classic Haircut", "description": "Traditional haircut with clippers and scissors", "duration_minutes": 30, "price": 25.0},
        {"id": "service_2", "name": "Haircut + Beard Trim", "description": "Full haircut with beard shaping", "duration_minutes": 45, "price": 35.0},
        {"id": "service_3", "name": "Premium Cut + Hot Towel", "description": "Deluxe haircut with hot towel treatment", "duration_minutes": 60, "price": 50.0},
        {"id": "service_4", "name": "Kids Cut", "description": "Haircut for children under 12", "duration_minutes": 20, "price": 15.0},
    ]
    services = [{**s, "shop_id": shop_id, "active": True, "created_at": now_iso} for s in service_defs]
    ...
    consent_ts = now_iso
    client_defs = [
        {"id": "client_1",  "name": "John Smith",      "phone": "+15559876543", "email": "john@example.com",    "no_shows": 0},
        {"id": "client_2",  "name": "Mike Wilson",      "phone": "+15551112222", "email": "mike@example.com",    "no_shows": 2},
        {"id": "client_3",  "name": "James Brown",      "phone": "+15553334444", "email": "james@example.com",   "no_shows": 0},
        {"id": "client_4",  "name": "Robert Taylor",    "phone": "+15554445555", "email": "robert@example.com",  "no_shows": 0},
        {"id": "client_5",  "name": "Carlos Rivera",    "phone": "+15555556666", "email": "carlos@example.com",  "no_shows": 1},
        {"id": "client_6",  "name": "Daniel Kim",       "phone": "+15556667777", "email": "daniel@example.com",  "no_shows": 0},
        {"id": "client_7",  "name": "Marcus Thompson",  "phone": "+15557778888", "email": "marcus@example.com",  "no_shows": 0},
        {"id": "client_8",  "name": "Andre Williams",   "phone": "+15558889999", "email": "andre@example.com",   "no_shows": 2},
        {"id": "client_9",  "name": "Kevin Patel",      "phone": "+15559990000", "email": "kevin@example.com",   "no_shows": 0},
        {"id": "client_10", "name": "Tyler Jackson",    "phone": "+15550001111", "email": "tyler@example.com",   "no_shows": 0},
    ]
    clients = []
    ...
    for day_offset in range(30, 0, -1):
        ...
        for slot in range(daily_count):
            ...
            apt = {
                ...
                "status": status,
                "price": service["price"] if status == AppointmentStatus.COMPLETED.value else 0,
                "deposit_required": random.random() < 0.3,
                "deposit_amount": 20.0 if random.random() < 0.3 else 0,
                "deposit_paid": status == AppointmentStatus.COMPLETED.value and random.random() < 0.3,
    ...
    upcoming_apts = [
        {"id": "apt_today_1", "shop_id": shop_id, "client_id": "client_1", "barber_id": "barber_1", "service_id": "service_1",
         "scheduled_at": future(hours=2), "duration_minutes": 30, "status": AppointmentStatus.CONFIRMED.value,
         "price": 0, "deposit_required": False, "deposit_amount": 0, "deposit_paid": False,
         "created_at": ago(days=1), "updated_at": now_iso},
        {"id": "apt_today_2", "shop_id": shop_id, "client_id": "client_4", "barber_id": "barber_2", "service_id": "service_2",
         "scheduled_at": future(hours=4), "duration_minutes": 45, "status": AppointmentStatus.DEPOSIT_PAID.value,
         "price": 0, "deposit_required": True, "deposit_amount": 20.0, "deposit_paid": True,
         "created_at": ago(days=2), "updated_at": now_iso},
        {"id": "apt_tomorrow_1", "shop_id": shop_id, "client_id": "client_3", "barber_id": "barber_1", "service_id": "service_3",
         "scheduled_at": future(days=1, hours=3), "duration_minutes": 60, "status": AppointmentStatus.PENDING.value,
         "price": 0, "deposit_required": False, "deposit_amount": 0, "deposit_paid": False,
         "created_at": ago(hours=5), "updated_at": now_iso},
    ]
    ...
    for cid, count in client_apt_counts.items():
        if count > 0:
            await db.clients.update_one({"id": cid}, {"$set": {"total_appointments": count}})
    ...
    for day_offset in [2, 4, 5, 7, 8, 10, 12, 13]:
        ...
        events.append({
            ...
            "client_id": client["id"],
            "data": {"service": service["name"], "original_client": "cancelled_client"},
    ...
    recovered_revenue = []
    for evt in events:
        if evt["event_type"] in (EventType.WAITLIST_FILLED.value, EventType.REVENUE_RECOVERED.value):
    ...
    for offset_min, direction, content in [
        ...
        (85, "outbound", "Hi John! Thanks for reaching out to Classic Cuts. We have availability today at 2pm or 4pm. Which works for you?"),
        (80, "inbound", "2pm works great"),
        (75, "outbound", "Your appointment is confirmed! Today at 2:00 PM - Classic Haircut with Marcus Johnson. Reply YES to confirm or NO to cancel."),
    ]:
        msg_counter += 1
        messages.append({
            "id": f"msg_{msg_counter}", "shop_id": shop_id, "client_id": "client_1",
            "direction": direction, "message_type": "sms", "content": content,
    ...
    for offset_min, direction, content in [
        ...
        (115, "outbound", "Got it, Robert! You're booked for tomorrow at 3:00 PM - Haircut + Beard Trim with David Lee. We'll send a reminder!"),
    ]:
        msg_counter += 1
        messages.append({
            "id": f"msg_{msg_counter}", "shop_id": shop_id, "client_id": "client_4",
            "direction": direction, "message_type": "sms", "content": content,
            "created_at": ago(minutes=offset_min),
        })
    ...
    for offset_hours, direction, content in [
        (26, "outbound", "Reminder: You have an appointment tomorrow at 11:00 AM - Classic Haircut. Reply YES to confirm or CANCEL to cancel."),
        (25, "inbound", "YES"),
        (25, "outbound", "Great! Your appointment is confirmed. See you tomorrow!"),
    ]:
        msg_counter += 1
        messages.append({
            "id": f"msg_{msg_counter}", "shop_id": shop_id, "client_id": "client_5",
            "direction": direction, "message_type": "sms", "content": content,
    ...
    for offset_hours, direction, content in [
        (72, "outbound", "Hi Mike, you missed your appointment today. Per our policy, a $20 no-show fee has been applied. Reply HELP for options."),
        (71, "inbound", "Sorry I forgot! Can I rebook?"),
        (70, "outbound", "No problem! A deposit of $20 will be required for your next booking. Reply BOOK to schedule."),
    ]:
        msg_counter += 1
        messages.append({
            "id": f"msg_{msg_counter}", "shop_id": shop_id, "client_id": "client_2",
            "direction": direction, "message_type": "sms", "content": content,
            "created_at": ago(hours=offset_hours),
        })
    ...
    for day_offset in [5, 7, 10, 14]:
        ...
        msg_counter += 1
        messages.append({
            "id": f"msg_{msg_counter}", "shop_id": shop_id, "client_id": client["id"],
        ...
        messages.append({
            "id": f"msg_{msg_counter}", "shop_id": shop_id, "client_id": client["id"],
            "direction": "inbound", "message_type": "sms", "content": "YES",
    ...
    # ── Waitlist ────────────────────────────────────────────
    waitlist = [
        {"id": "wait_1", "shop_id": shop_id, "client_id": "client_3", "service_id": "service_2", "preferred_date": future(days=2), "flexible_hours": 3, "active": True, "contact_attempts": 0, "created_at": ago(days=1)},
        {"id": "wait_2", "shop_id": shop_id, "client_id": "client_6", "service_id": "service_3", "preferred_date": future(days=3), "flexible_hours": 2, "active": True, "contact_attempts": 0, "created_at": ago(hours=12)},
        {"id": "wait_3", "shop_id": shop_id, "client_id": "client_9", "service_id": "service_1", "preferred_date": future(days=1), "flexible_hours": 4, "active": True, "contact_attempts": 1, "created_at": ago(hours=6)},
    ]
