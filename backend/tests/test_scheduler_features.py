...
class TestSchedulerStatus:
    ...
    def test_scheduler_status_returns_running(self, auth_token):
        """GET /api/jobs/status returns running scheduler with job IDs and next_run times"""
        ...
        # Verify jobs list exists
        assert "jobs" in data
        assert isinstance(data["jobs"], list)
        
    def test_scheduler_has_both_jobs(self, auth_token):
        """Verify both appointment_reminders and daily_summary jobs exist"""
        response = requests.get(
            f"{BASE_URL}/api/jobs/status",
            headers={"Authorization": f"Bearer {auth_token}"}
        )
        ...
        assert "appointment_reminders" in job_ids, "appointment_reminders job not found"
        assert "daily_summary" in job_ids, "daily_summary job not found"
        
    def test_scheduler_jobs_have_next_run_times(self, auth_token):
        """Verify jobs have next_run times and triggers"""
        response = requests.get(
            f"{BASE_URL}/api/jobs/status",
            headers={"Authorization": f"Bearer {auth_token}"}
        )
        ...
        for job in data["jobs"]:
            ...
            # next_run can be None if job just ran, but trigger should always exist
...
class TestDailySummary:
    ...
    def test_daily_summary_stats_data_types(self, auth_token):
        ...
        # Integer fields
        int_fields = [
            "total_appointments", "completed", "no_shows", "cancelled", "pending",
            "messages_sent", "messages_received", "waitlist_active", "new_clients"
        ]
